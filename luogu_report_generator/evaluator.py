...
def remove_injected_trusted_block(report_md: str) -> str:
    # 删除 AI 报告里任何形式的"可信数据块"（程序已经生成），
    # 包含 4 个 H2：数据校准与真实统计 / 掌握度判定标准（5 档）/
    # 知识树图谱 / 知识点明细
    md = report_md
    for h2 in (
        "## 数据校准与真实统计",
...
def normalize_report_markdown(report_md: str, export_data: dict | None = None) -> str:
    ...
    关键不变量（v3.9.52 照搬 oi.aijiangti.cn 14:07 原版 report.html）:
    1. 模板 `report_template.html` **已经**渲染了：
       ...
       → 所以 markdown 区域（`{{ report_html | safe }}`）第一段是
         3 个程序生成的 H2（数据校准与真实统计 + 掌握度判定标准 + 知识树图谱），
    ...
    2. AI 报告**必须**从 `# 🏅 OI 竞赛选手深度能力诊断与训练报告` (H1) 开始,
       ...
       **不要**写程序已生成的可信数据块的任何 H2/H3。
       ...
       然后把 `build_trusted_data_summary_md()` 输出**插到**最前面（3 个 H2），
...
请你输出一份结构化的 Markdown 辅导报告，必须包含以下部分。在生成 Markdown 时，请务必使用以下视觉元素增强表现力：
 ...
 4. **【六维能力雷达表与诊断】（评分参考：85-100 优秀 | 65-84 良好 | 40-64 基础 | <40 薄弱）**：
  ...
  5. **【考纲精准定级与知识点盲区】**（根据提供的 NOI大纲 2025版）：
     ...
     - **知识点覆盖与树状图**：绝对**不要**再写"知识点明细"、"知识点覆盖统计表"、"知识树"、"CSP-NOI 2025 大纲知识点对标"、按级别逐项罗列 AC 数 / 难度的明细表等任何形式的明细章节！这些都由程序自动生成在"数据校准与真实统计"H2 之下的"知识树图谱"小节中（4 棵 SVG 树 + 4 行汇总表 + 4 个级别徽章）。你**只能**用 1-2 段话**综合点评**："哪些大分支（4 大等级）覆盖得好、哪些几乎为零，并给 1-2 条具体训练建议"即可。**严禁**把上文 `### 大纲知识点对标` 块里的原始 markdown 表格 / `### 入门级 CSP-J` 之类的表头原样搬运到你的章节里。
